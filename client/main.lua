		...
		for _, door in pairs(doors) do
			...
			if double then
				if door.distance < 80 then
					...
					for i = 1, 2 do
						local entity = double[i].entity
						if entity then
							if UnregisterDoorTargetEntity then UnregisterDoorTargetEntity(entity) end
							DoorEntity[entity] = nil
						end
						double[i].entity = nil
					end
				end
...
RegisterNetEvent('ox_doorlock:setState', function(id, state, source, data)
	...
	if Config.Notify and source == cache.serverId then
		if state == 0 then
			Node7DoorNotify(locale('unlocked_door'), 'success')
		else
			Node7DoorNotify(locale('locked_door'), 'success')
		end
	end
		...
		for i = 1, 2 do
			...
			if data then
				...
				if double[i].entity then
					if UnregisterDoorTargetEntity then UnregisterDoorTargetEntity(double[i].entity) end
					DoorEntity[double[i].entity] = nil
				end
			end
		end
	else
...
if Config.DoorPrompt and Config.DoorPrompt.Enabled then
...
CreateThread(function()
	...
	if drawSprite then
		...
		if sprite1 then
            ...
            while not HasStreamedTextureDictLoaded(sprite1) and GetGameTimer() < timeout do Wait(0) end
		end

		if sprite2 then
			RequestStreamedTextureDict(sprite2, true)
            local timeout = GetGameTimer() + 5000
            while not HasStreamedTextureDictLoaded(sprite2) and GetGameTimer() < timeout do Wait(0) end
		end
	end
	...
	while true do
			...
			for i = 1, num do
				...
				if door.distance < door.maxDistance then
					...
					if drawSprite and not door.hideUi then
						...
						if sprite and doorEntity and doorEntity ~= 0 then
							local doorBone = getDoorHandPoint(doorEntity)
							if doorBone.x == 0 and doorBone.y == 0 and doorBone.z == 0 then
								doorBone = getEntityCenterCoords(doorEntity)
							end

							SetDrawOrigin(doorBone.x, doorBone.y, doorBone.z + 0.04)
							...
							if door.distance < (door.maxDistance / 2) then
								...
								DrawSprite(sprite[1], 'point', sprite[3], sprite[4], 0.017, 0.034, sprite[7], sprite[8], sprite[9], sprite[10], sprite[11])
							end
		...
		if canUseClosest then
			...
			-- Direct control fallback keeps hidden-UI doors and servers without prompts usable.
			local controlReleased = IsControlJustReleased(0, Config.DoorPrompt and Config.DoorPrompt.Control or 0xCEFD9220)

			if promptCompleted or controlReleased then
				useClosestDoor()
			end
		elseif showUI then
			lib.hideTextUI()
